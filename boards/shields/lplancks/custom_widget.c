...
static const struct zmk_led_hsb COLOR_CAPS = {0, 0, 100};
...
static int caps_lock_listener_cb(const zmk_event_t *eh) {
    ...
    // Forza l'aggiornamento del colore del LED di base
    if (initialized) {
        update_layer_color();
    }
...
void update_layer_color(void) {
	if (caps_lock_on) {
        if (led_layer_color != 8) {
             led_layer_color = 8;
             struct blink_item color = {.color = led_layer_color};
             LOG_INF("Setting LED color to CAPS LOCK");
...
static void set_rgb_leds(int8_t color, uint16_t duration_ms) {
    ...
    if(color == COLOR_RESET && led_current_color != COLOR_RESET){
        ...
        // Parte nuova: scegli il colore CAPS se l'indice è 8
        if (color == 8) {
            zmk_rgb_underglow_set_hsb(COLOR_CAPS);
        } else {
